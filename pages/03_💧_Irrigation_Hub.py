...
try:
    if os.path.exists(bg_image_path):
        ...
        st.markdown(f"<p style='color: #F44336; font-weight: 600;'>{t('bg_err')}: {bg_image_path}</p>", unsafe_allow_html=True)
        bg_style = "background: linear-gradient(135deg, #E8F5E9 0%, #C8E6C9 100%) !important;"
except Exception as e:
    st.markdown(f"<p style='color: #F44336; font-weight: 600;'>{t('bg_load_err')}: {e}</p>", unsafe_allow_html=True)
    bg_style = "background: linear-gradient(135deg, #E8F5E9 0%, #C8E6C9 100%) !important;"
